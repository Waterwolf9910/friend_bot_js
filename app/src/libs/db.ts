...
if (config.DBType.toLowerCase() == "sqlite" || config.DBType.toLowerCase() == "sqlite3") {
    ...
    sequelize = new _sequelize.Sequelize({ username: config.DBUser, database: config.DBName, password: config.DBPassword, dialect: "mariadb", dialectModule: require("mariadb"), host: config.DBHost, port: config.DBPort, pool: {acquire: 20000}, retry: {max: 5}, ssl: config.DBUseSSL, define: { freezeTableName: true, charset: '"utf8"' }, logging: log })
} else if (config.DBType.toLowerCase() == "mysql" || config.DBType.toLowerCase() == "mysql2") {
    sequelize = new _sequelize.Sequelize({ username: config.DBUser, database: config.DBName, password: config.DBPassword, dialect: "mysql", dialectModule: require("mariadb"), host: config.DBHost, port: config.DBPort, pool: { acquire: 20000 }, retry: { max: 5 }, ssl: config.DBUseSSL, define: { freezeTableName: true, charset: '"utf8"' }, logging: log })

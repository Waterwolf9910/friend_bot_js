...
let start = async (secret: string, client_secret: string, config: import("main/types").Config, client: import("discord.js").Client<true>) => {
    ...
    if (!isDev) {
        ...
        let wpconfig: import("webpack").Configuration = __non_webpack_require__("../builder_resources/webpack.config")
...
let setupRoutes = (config: import("main/types").Config, client_secret: string, client: import("discord.js").Client<true>) => {
    ...
    app.get("/getLogin", async (req, res) => {
        ...
        req.session.save((err) => {
            ...
            res.type('text').end(`https://discord.com/oauth2/authorize?${params.toString()}`)
    ...
    app.get('*',  async (req, res) => {
        ...
        fs.readFile(path.normalize(`${__dirname}/../static/${req.path}`), {}, (err, data) => {
            if (!err) {
                ...
                return res.type(type ?? 'text').send(path.normalize(`${__dirname}/../static/${req.path}`))
            }

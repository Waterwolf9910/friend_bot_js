...
let _: import("main/types").Command= {
    ...
    slash: require("./slash").addSubcommand(sub => {
        sub.setName("set")
        sub.setDescription("sets xp of a user")
    ...
    level: "admin",
    description: "Sets xp for a user",
    usage: "xp set"
}
let run = async (guild: discord.Guild, member: discord.GuildMember, selected_member: discord.GuildMember, channel: discord.GuildTextBasedChannel, amount: number): Promise<import("main/types").CommandResult> => {
    ...
    xp[selected_member.id] = amount
    guild_config.xp = xp

    try {
        await guild_config.save()
        channel.send(`${selected_member.displayName} now has ${amount} xp (was ${current_xp})`)
    } catch {

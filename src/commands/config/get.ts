...
let run = async (guild: discord.Guild, member: discord.GuildMember, channel: discord.GuildTextBasedChannel, key: string): Promise<import('../../types').CommandResult> => {
    ...
    let updateConfig = () => {
        ...
        msg.awaitReactions({ time: 300000, maxEmojis: 1, filter: (re, user) => user.id == member.id, errors: [ "time" ] }).then(async col => {
            ...
            msg = await msg.edit({embeds: [pages[activePage]]})
            updateConfig()
        }).catch(() => {
            msg.delete().catch(() => null)
        })

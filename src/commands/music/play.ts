...
let play_next = async (text_channel: import("discord.js").GuildTextBasedChannel, guildId: string, retrys = 0) => {
    ...
    if (queueInfo.clearing) {
        ...
        let timeout = setTimeout(() => {
            try {
                queueInfo.player?.removeAllListeners()
            } catch { }
            end(guildId)
...
let run = async (member: import('discord.js').GuildMember, guildId: string, reply: (message: string | import('discord.js').MessageCreateOptions | import('discord.js').MessagePayload) => Promise<import('discord.js').Message>, text_channel: import('discord.js').GuildTextBasedChannel, search: string, searchSplit: string[]): Promise<import('../../types').CommandResult> => {
    ...
    if (url) {
        if (ytpl.validateID(url.searchParams.get("list"))) {
            ...
            let _video = await ytdl.getInfo(searchSplit[ 0 ]);
            let video = _video.videoDetails
            if (!video) {
            ...
            if (!video.isLiveContent) {
                queue.push({
                    ...
                    link: searchSplit[ 0 ],
                    source: "Youtube",
                    thumbnail: _video.thumbnail_url,
                    title: video.title,
                    uploader: {
                        name: video.author.name,
    ...
    if (player.listenerCount("stateChange") < 1) {
        player.on("stateChange", async (oldS, newS) => {
